...
***   " IF sy-opsys EQ 'AIX'. diganti untuk disesuai dengan upgrade SOH - 10 juni 2024
    IF sy-opsys EQ 'AIX' OR sy-opsys EQ 'Linux' OR sy-opsys EQ 'LINUX'.
...
.
  ...
  OPEN DATASET l_file_out FOR INPUT IN TEXT MODE ENCODING DEFAULT.
  IF sy-subrc NE 0.
    OPEN DATASET l_file_out FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.
  ELSE.
    CLOSE DATASET  l_file_out.
    OPEN DATASET l_file_out FOR APPENDING IN TEXT MODE ENCODING DEFAULT.
  ENDIF.
  IF sy-subrc NE 0.
    WRITE: / 'Error Create file log : ', l_file_out.
    RETURN.
  ENDIF.
