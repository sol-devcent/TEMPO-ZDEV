...
FORM f_build_fieldcat TABLES ft_report.
  ...
  PERFORM f_fieldcatg USING ft_report:
    'NUMBER' '' '' '' '6' 'No.' '' '' ''
    '' '' '' '' '' 'X' '',
    'KTEXT' 'AUFK' 'KTEXT' '' '' 'Jenis Barang yg diproduksi' '' '' ''
    '' '' '' '' '' '' '',
    'SAWAL' 'S933' 'MENGE' '' '' 'Saldo Awal' '' '' '' '' '' '' 'BASME'
    '' 'X' '',
    'BASME' 'S933' 'BASME' 'X' '' 'Satuan' '' '' '' '' '' '' '' '' ''
    '',
    'SMASUK' 'S933' 'MENGE' '' '' 'Pemasukan' 'X' '' '' '' '' '' 'BASME'
    '' 'X' '',
    'SJUMLAH' 'S933' 'MENGE' '' '' 'Jumlah' '' '' '' '' '' '' 'BASME'
    '' 'X' '',
    'SGUNA' 'S933' 'MENGE' '' '' 'Yg Digunakan' 'X' '' '' '' '' ''
    'BASME' '' 'X' 'X',
    ...
    '' '' '',
    'SPROD2' 'S933' 'MENGE' '' '35'
    'Hasil akhir dalam satuan terkecil' '' '' '' '' '' ''
    'BASME2' '' 'X' 'X',
    'BASME2' 'S933' 'BASME' '' '' 'Satuan Terkecil' '' '' '' '' '' '' ''
    '' '' '',
    'KETR' '' '' '' '' 'Keterangan' '' '' '' '' '' '' '' '' '' ''.
...
FORM f_hdr_line4 USING    value(p_0617).
  DATA: l_start          TYPE i,
        ...
        l_ort01(100)     VALUE '                       ',
        l_period(100)    VALUE 'PERIODE : ',
        ld_mnr LIKE t247-mnr,
        ld_ltx LIKE t247-ltx,
*        ld_kep(100) VALUE  'Keputusan Menteri Keuangan RI',
        ld_kep(100) VALUE  'Keputusan Menteri Keuangan RI      LACK-4',
        ld_nom(100), "VALUE  'Nomor: Kep-078/BC.4/2005',
        ld_tang(100), "VALUE 'Tanggal: 13 Januari 2005',
        ld_lamp(100) VALUE 'LACK-4'.

*---Nomor & Tanggal Kep Menteri
  CLEAR ld_ltx.
  SELECT SINGLE mnr ltx INTO (ld_mnr, ld_ltx)
                        FROM t247
                        WHERE mnr = p_tgl+4(2) AND
                              spras = 'i'.
  CONCATENATE 'Tanggal:' p_tgl+6(2) ld_ltx p_tgl(4) INTO ld_tang
    SEPARATED BY space.
  CONCATENATE 'Nomor:' p_nom INTO ld_nom SEPARATED BY space.
...
*  COMPUTE l_right2 = sy-linsz - l_length.
  COMPUTE l_right2 = sy-linsz - l_length - 12.
  COMPUTE l_length = strlen( ld_tang ).
  COMPUTE l_right3 = sy-linsz - l_length.
  COMPUTE l_right3 = sy-linsz - l_length - 12.
  COMPUTE l_length = strlen( ld_lamp ).
  COMPUTE l_right0 = sy-linsz - l_length.

*---Title
  l_title = 'LAPORAN PENGGUNAAN ETIL ALKOHOL'.
...
*---Period
  ...
  SELECT SINGLE mnr ltx INTO (ld_mnr, ld_ltx)
                        ...
                        WHERE mnr = p_period+4(2) AND
                              spras = 'i'.
  CONCATENATE l_period ld_ltx p_period(4) INTO l_period
    SEPARATED BY space.
...
*    SEPARATED BY space.
*-Changed by Budi: VA_STRAS to VA_STREET
  CONCATENATE l_alamat va_street INTO l_alamat
    SEPARATED BY space.
  CONCATENATE l_ort01 va_ort01 INTO l_ort01
    SEPARATED BY space.

*--- output line
*  WRITE: /l_right0 ld_lamp,
  WRITE: /l_right1 ld_kep,
         /l_right2 ld_nom,
         /l_right3 ld_tang,
         /l_start l_title,
         /l_start l_period.
...
*      -->P_1442   text
*----------------------------------------------------------------------*
FORM f_ftr_line3_1 USING    value(p_1442).
...
*  WRITE p_tgl DD/MM/YYYY TO l_date.
  ...
  DO 5 TIMES.
    CASE sy-index.
      WHEN 1.
        WRITE: /60  p_docno1,
                84  p_qty1 RIGHT-JUSTIFIED," NO-ZERO,
                104 p_tgl1 NO-ZERO.
        IF p_docno1 IS INITIAL.
          SKIP.
        ENDIF.
      WHEN 2.
        WRITE: /60  p_docno2,
                84  p_qty2 RIGHT-JUSTIFIED," NO-ZERO,
                104 p_tgl2 NO-ZERO.
        IF p_docno2 IS INITIAL.
          SKIP.
        ENDIF.
      WHEN 3.
        WRITE: /60  p_docno3,
                84  p_qty3 RIGHT-JUSTIFIED," NO-ZERO,
                104 p_tgl3 NO-ZERO.
        IF p_docno3 IS INITIAL.
          SKIP.
        ENDIF.
      WHEN 4.
        WRITE: /60  p_docno4,
                84  p_qty4 RIGHT-JUSTIFIED," NO-ZERO,
                104 p_tgl4 NO-ZERO.
        IF p_docno4 IS INITIAL.
          SKIP.
        ENDIF.
      WHEN 5.
        WRITE: /60  p_docno5,
                84  p_qty5 RIGHT-JUSTIFIED," NO-ZERO,
                104 p_tgl5 NO-ZERO.
        IF p_docno5 IS INITIAL.
          SKIP.
        ENDIF.
    ENDCASE.
  ENDDO.
