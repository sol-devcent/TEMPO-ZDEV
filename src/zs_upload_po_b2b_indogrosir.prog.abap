...
*  c_interfaceerror(125)   TYPE c VALUE '\error',
...
*  c_interfaceerror_aix(125)   TYPE c VALUE '/error',
...
*}   REPLACE
    p_path = '/interface/B2B/indogrosir'.
  ENDIF.
...
*}   REPLACE
    ...
    CONCATENATE v_logfile '/' sy-datum(4) '/' sy-datum+4(2) '/idg' INTO v_logfile.
  ELSE.
...
** Call program ZS_CREATE_QUOTATION_B2B
  ...
  LOOP AT i_zsh_b2b.
    rspar_line-selname = 'S_VKBUR'.
    rspar_line-kind    = 'S'.
    rspar_line-sign    = 'I'.
    rspar_line-option  = 'EQ'.
    rspar_line-low     = i_zsh_b2b-vkbur.
    APPEND rspar_line TO rspar_tab.

    rspar_line-selname = 'S_EBELN'.
    rspar_line-kind    = 'S'.
    rspar_line-sign    = 'I'.
    rspar_line-option  = 'EQ'.
    rspar_line-low     = i_zsh_b2b-ebeln.
    APPEND rspar_line TO rspar_tab.
  ENDLOOP.

**  SUBMIT zs_create_quotation_b2b WITH SELECTION-TABLE rspar_tab
**                                 AND RETURN.
