...
AT SELECTION-SCREEN ON pa_bukrs.
  IF pa_bukrs EQ '8020' OR pa_bukrs EQ '8010' OR pa_bukrs EQ '8030'.
  ELSE.
    MESSAGE e000(zs)
      WITH 'Company Code must be entry (8010, 8020, 8030)'.
  ENDIF.


AT SELECTION-SCREEN ON pa_vkbur.
  IF pa_bukrs EQ '8020'.
    IF pa_vkbur EQ 0 OR pa_vkbur EQ space OR pa_vkbur+0(2) NE '02'.
    ...
    IF val IS INITIAL.
      ...
      LOOP AT i_itab3 INTO wa_itab1.
        ...
        IF wa_itab1-shkzg = 'H'.
          wa_itab1-wrbtr = wa_itab1-wrbtr * -1.
        ENDIF.
        MOVE wa_itab1-bukrs TO i_itab1-bukrs.
...
FORM f_get_data.
  ...
  SELECT a~bukrs a~hkont a~gjahr a~belnr a~budat a~bldat a~kunnr
         a~waers a~xblnr a~monat a~shkzg a~wrbtr a~zfbdt a~zbd1t
         a~buzei a~gsber a~zlspr a~zuonr a~xref1 a~xref2 a~xref3
         a~vbund a~blart
         b~vkbur b~spart
         d~pernr
      ...
      WHERE a~bukrs EQ pa_bukrs                           AND
            ...
            a~zuonr IN do_no                              AND
            a~belnr IN so_belnr                           AND
            a~zlspr IN (space,'Z','B')                    AND
            a~blart IN ('DA', 'RV','DR','DG', 'ZA', 'DZ') AND
...
*---------------------------------------------------------------------*
FORM f_init_print.
  w1   =   4.      w11 = 11.      w21 = 12.      w31 = 10.
  w2   =  11.      w12 = 11.      w22 = 10.      w32 = 12.
...
*----------------------------------------------------------------------*
FORM get_zfbid.
  SELECT bukrs gjahr vbeln bbeln fkdat kunnr wrbtr zuonr
         zfbdt buzei erdt2
