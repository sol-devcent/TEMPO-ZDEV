...
*                                                                      *
************************************************************************
REPORT zmr_psikotropika1 MESSAGE-ID zs
                         LINE-COUNT 70
                         LINE-SIZE 204
...
DATA  BEGIN OF gt_movement OCCURS 1.
DATA:    matnr   LIKE s031-matnr,
         mzubb   LIKE s031-mzubb,
         magbb   LIKE s031-magbb.
DATA  END   OF gt_movement.
...
PARAMETERS:
  p_nama(20) OBLIGATORY DEFAULT sy-uname,
...
*------------------------------------------------------*
  SELECT mara~matnr meins ersda maktx
  FROM ( mara INNER JOIN makt ON makt~matnr = mara~matnr )
  INTO CORRESPONDING FIELDS OF TABLE t_okt
  WHERE ( profl = 'OKT' OR profl = 'PSI' )
    AND mtart = 'ZPHA'. " JANGAN LUPA OKT PASTI PHARMA
...
*------------------------------------------------------*
  ...
  LOOP AT i_all INTO wa_all.
    ...
    IF sy-subrc EQ 0.
      CASE wa_invo-flag.
        WHEN 'D'.
          lv_menge  = wa_all-menge.
          lv_matnr  = wa_all-matnr.
          DELETE i_all.
          READ TABLE i_all INTO wa_all WITH KEY matnr = lv_matnr.
          IF sy-subrc = 0.
            wa_all-mzubb = wa_all-mzubb + lv_menge.
...
*    IZIN = '31081/PBF/CAP-20/IX/96'.
  ...
  WRITE: /78 'LAPORAN DISTRIBUSI SEDIAAN PSIKOTROPIKA'.
  SKIP.
