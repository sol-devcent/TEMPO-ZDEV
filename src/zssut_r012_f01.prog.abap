...
*&---------------------------------------------------------------------*
 FORM f_get_data .
   ...
   zssutdt021-begda = s_datum-low.
   gs_header-tanggal = s_datum-low.
   PERFORM f_get_dayname USING s_datum-low 'FULL' CHANGING gs_header-hari.
...
***            AND   kunn2 IN s_kunn2
   ...
   IF p_chg = 'X'.
     SELECT SINGLE * FROM zssutdt025 WHERE vkorg = p_vkorg
                                       AND vkbur = p_vkbur
                                       AND pernr = p_pernr
                                       AND sdate = s_datum-low
                                       AND status NE 'D'
                                       AND str_dcp_dat = s_datum-low
                                       AND end_dcp_dat = s_datum-high.
   ELSE.
     SELECT SINGLE * FROM zssutdt025 WHERE vkorg = p_vkorg
                                       AND vkbur = p_vkbur
                                       AND pernr = p_pernr
...
*       APPEND INITIAL LINE TO gt_itab.
   ...
   ELSE.
     ...
     DATA: lv_time TYPE i, lv_date LIKE sy-datum.
     lv_time = s_datum-high - s_datum-low + 1.
     lv_date = s_datum-low.
     LOOP AT gt_022.
...
*** Cek tanggal untuk menentukan tgl kunjunagn sesuai dengan table ZSSUTDT022
       lv_date = s_datum-low.
       DO lv_time TIMES.
         PERFORM f_get_dayname USING lv_date '' CHANGING lv_dname.
         PERFORM f_get_weeknumber1 USING lv_date CHANGING lv_week.
         CONCATENATE lv_dname lv_week INTO lv_fname. CONDENSE lv_fname.
         CONCATENATE 'GT_022-' lv_fname INTO lv_fname. CONDENSE lv_fname.
         ASSIGN (lv_fname) TO <fs>.
         IF <fs> = 'X'.
...
*&---------------------------------------------------------------------*
 FORM f_update_025 USING fu_release.
   IF gv_mode = 'CRT'.
     ...
     zssutdt025-umjah   = s_datum-low(4). "zssutdt023-umjah.
     zssutdt025-sdate   = s_datum-low.
     ...
     zssutdt025-str_dcp_dat = s_datum-low. "p_datum.
     zssutdt025-end_dcp_dat = s_datum-high. "p_enddcp.
...
*&---------------------------------------------------------------------*
 FORM f_update_026 .
   ...
   ELSEIF gv_mode = 'UPD'.
     ...
     ENDIF.
     SUBMIT zsfasd_i0001 WITH p_vkorg = p_vkorg
                         WITH p_vkbur = p_vkbur
                         WITH s_pernr-low = p_pernr
                         WITH s_call-low = zssutdt025-daily_call_num.
   ENDIF.
...
*&---------------------------------------------------------------------*
 FORM f_modify_data .
   ...
   LOOP AT gt_itab WHERE kunnr IS NOT INITIAL.
     ...
     "zssutdt026-umjah             = p_datum+0(4).
     zssutdt026-umjah             = s_datum-low+0(4).
...
**** Cek cara update kunjungan ke table ZSSUTDT026 berdasarkan table ZSSUTDT022.
     lv_date = s_datum-low.
     CLEAR zssutdt026-master_stat_indi.
     DO lv_time TIMES.
       " __* is the data same as in the master visitation matrix ?
     ...
     IF zssutdt026-call_date IS INITIAL.
       zssutdt026-call_date = s_datum-low.
     ENDIF.
     APPEND zssutdt026 TO lt_026.
...
*&---------------------------------------------------------------------*
 FORM f_populate_data .
   ...
   CONCATENATE p_vkbur '-' s_datum-low+0(4) '-'zssutdt025-daily_call_num INTO gs_header-kode_dok.
         ...
         IF p_del = 'X'.
           SELECT SINGLE * FROM zssutdt025
                 WHERE vkorg = p_vkorg AND
                       ...
                       pernr = p_pernr AND
                       "umjah = p_datum(4) AND
                       daily_call_num EQ s_dcp-low
                       "daily_call_num = p_dcp1 AND
                       "str_dcp_dat IN s_datum. "  AND
                        AND str_dcp_dat = s_datum-low
                        AND end_dcp_dat = s_datum-high.
           "zrelease = 'X'. " AND zprint NE 'X'.
         ENDIF.
...
****END_DCP_DAT ZEND_DCP  DATS  8 0 End DCP Date
   ...
   PERFORM f_fieldcatg USING ft_report:
     ...
     'PERNR'          'ZSSUTDT025' 'PERNR'          '' '' 'No. Salesmen'   '' '' '' '' '' '' '' '',
     'ENAME'          'PA0001'     'ENAME'          '' '' 'Salesmen Name'   '' '' '' '' '' '' '' '',
