...
*      AND a~spras = sy-langu.
  ...
  SELECT a~matnr maktx meins normt INTO TABLE gt_makt2
    FROM makt AS a JOIN mara AS b ON a~matnr = b~matnr
    WHERE a~matnr IN s_matnr
      AND a~spras = sy-langu.
...
*    gw_opnstk-LGORT = lt_s035-lgort.
    ...
    MOVE-CORRESPONDING gw_opnstk TO gw_opnstk2.
    CLEAR gw_opnstk2-charg.
    COLLECT gw_opnstk2 INTO gt_opnstk2.
    CLEAR: gw_opnstk,gw_opnstk2.
  ENDLOOP.
...
FORM f_get_transaction .
  ...
  CASE 'X'.
    WHEN p_depkes.
      DELETE gt_mseg WHERE bwart = '919'.
    WHEN OTHERS.
  ENDCASE.
...
FORM f_append_init_lines2  USING    fu_matnr.
  CLEAR: gt_ztspmmdt002,gt_makt,gt_makt2,gt_lfa1.
  READ TABLE gt_ztspmmdt002 WITH KEY matnr = fu_matnr.
  READ TABLE gt_makt WITH KEY matnr = fu_matnr.
  READ TABLE gt_makt2 WITH KEY matnr = fu_matnr.
*  READ TABLE gt_lfa1 WITH KEY lifnr = gt_ztspmmdt002-lifnr.
  ...
  <fs_kemenkes>-maktx2         = gt_makt2-maktx.
  <fs_kemenkes>-meins          = gt_makt2-meins.
...
** Pemasukan
    IF gt_mseg-bwart IN gr_in AND
      ...
      CASE gt_mseg-bwart.
        ...
        WHEN '653' OR '654' OR '655' OR '656' OR 'Z13' OR 'Z14' OR '913' OR '914'.
          CLEAR: gt_kna1_655.
...
** Pengeluaran
    IF gt_mseg-bwart IN gr_out AND
      ...
      CASE gt_mseg-bwart.
        ...
        WHEN '303' OR '304' OR '641' OR '642' OR '645' OR '646' OR 'Z47' OR 'Z48' OR 'Z41' OR 'Z42'.
          CLEAR: gt_mseg_641,gt_t001w.
          READ TABLE gt_mseg_641 WITH KEY mblnr = gt_mseg-mblnr
                                          mjahr = gt_mseg-mjahr
                                          parent_id = gt_mseg-line_id.
...
*                                          charg = gt_mseg-charg.
        ...
        WHEN '555' OR '556' OR '920' OR '921' OR '922' OR '923' OR '926' OR '927'.
          CLEAR: gt_t001w.
...
FORM f_process_data_kemenkes .
  ...
  DATA: lv_adrnr2 LIKE lfa1-adrnr,
  ...
  LOOP AT gt_opnstk2 INTO gw_opnstk2.
    PERFORM f_append_init_lines2 USING gw_opnstk2-matnr.
    <fs_kemenkes>-saw_jumlah = gw_opnstk2-labst.
...
*       gt_mseg-shkzg EQ 'S'.
      ...
      IF ( gt_mseg-bwart = '311' OR gt_mseg-bwart = '301' ) AND
         gt_mseg-shkzg = 'H'.
      ELSEIF ( gt_mseg-bwart NE '311' AND gt_mseg-bwart NE '301' ) AND
               gt_mseg-xauto = 'X'.
      ELSE.
        ...
        CLEAR: lv_code,lv_name1,lv_ket,lv_type.
        CASE gt_mseg-bwart.
          WHEN '101' OR '102'.
            CLEAR: gt_ekko,gt_lfa1_101,gt_t001w.
            READ TABLE gt_ekko WITH KEY ebeln = gt_mseg-ebeln.
...
*      ADD 1 TO lv_norut2.
        ...
        IF gt_mseg-bwart IN gr_in0.
          IF gt_mseg-bwart = '101' OR gt_mseg-bwart = '102'.
            ...
            ELSE.
              ...
              TRANSLATE lv_remark TO UPPER CASE.
              IF lv_remark(1) = 'P'.
                lv_type = 'PBF'.
              ELSE.
                lv_type = 'IF'.
        ...
        ELSEIF gt_mseg-bwart IN gr_in1.
          ...
          ELSEIF gt_mseg-bwart = '311' OR gt_mseg-bwart = '301'.
            lv_type = 'PBF'.
          ELSE.
            lv_type = 'PBF'.
          ENDIF.
        ...
        READ TABLE gt_kemenkes ASSIGNING <fs_kemenkes>
                               WITH KEY matnr = gt_mseg-matnr
                                        ...
                                        in_code = lv_code.
        IF sy-subrc = 0.
          PERFORM f_update_line USING lv_code lv_seqno lv_type.
        ...
        ELSE.
          ...
          CASE lv_type.
            WHEN 'IF'.
              <fs_kemenkes>-in_pabrik_code = lv_code.
              <fs_kemenkes>-in_pabrik_name = lv_name1.
            WHEN 'PBF'.
              <fs_kemenkes>-in_pbf_code = lv_code.
              <fs_kemenkes>-in_pbf_name = lv_name1.
          ENDCASE.
        ENDIF.
      ENDIF.
    ENDIF.
...
*        ADD 1 TO lv_norut3.
          ...
          TRANSLATE lv_name1 TO UPPER CASE.
          IF lv_type NE 'PBF' OR
             ( lv_type EQ 'PBF' AND lv_name1(1) NE 'P' ).
            CLEAR lv_code.
          ENDIF.
...
*                                        out_name1 = lv_name1.
          IF sy-subrc = 0.
            PERFORM f_update_line2 USING lv_seqno lv_type lv_name1.
...
*              <fs_kemenkes>-in_pabrik_name = lv_name1.
              WHEN 'PBF'.
                <fs_kemenkes>-out_pbf_code = lv_code.
                <fs_kemenkes>-out_pbf_name = lv_name1.
            ENDCASE.
          ENDIF.
        ENDIF.
      ENDIF.
  ...
  IF gt_kemenkes[] IS NOT INITIAL.
    ...
    SORT gt_kemenkes BY matnr norut.
    LOOP AT gt_kemenkes.
      CLEAR: gt_a510,gt_konp.
      ...
      MODIFY gt_kemenkes TRANSPORTING nilai.


      AT END OF matnr.
        PERFORM f_modify_stock_opname USING gt_kemenkes-matnr.
        PERFORM f_delete_initial_row USING gt_kemenkes-matnr.

        READ TABLE gt_kemenkes WITH KEY matnr = gt_kemenkes-matnr
        ...
        IF sy-subrc = 0.
          ...
          gt_kemenkes-norut = '900'.
          MODIFY gt_kemenkes TRANSPORTING sak_jumlah norut.
        ENDIF.
      ENDAT.
    ENDLOOP.
  ENDIF.
...
*&      Form  F_UPDATE_LINE
*&---------------------------------------------------------------------*
FORM f_update_line USING fu_code fc_seqno fc_type.
...
*&      Form  F_UPDATE_LINE2
*&---------------------------------------------------------------------*
FORM f_update_line2 USING fc_seqno fc_type fc_name1.
  IF fc_type = 'PBF'.
    IF fc_name1(1) = 'P'.
...
FORM f_get_data_excel_kemenkes .
  ...
  LOOP AT gt_kemenkes.
    ...
    gs_zmmst_kemenkes-desc_produk           = gt_kemenkes-maktx2.
    gs_zmmst_kemenkes-uom_produk            = gt_kemenkes-meins.
...
*    gs_zmmst_kemenkes-keterangan_keluar     = gt_kemenkes-out_ket.
    gs_zmmst_kemenkes-kode_pabrik           = gt_kemenkes-in_pabrik_name.
    gs_zmmst_kemenkes-kode_pbf              = gt_kemenkes-in_pbf_name.
    gs_zmmst_kemenkes-kode_pbfout           = gt_kemenkes-out_pbf_name.
...
FORM f_list_data_excel_kemenkes .
  ...
  LOOP AT gt_zmmst_kemenkes INTO gs_zmmst_kemenkes.
    ...
    lo_worksheet->set_cell_style( ip_column = 'K' ip_row = row ip_style = lv_style_right_guid ).
    ...
    lo_worksheet->set_cell_style( ip_column = 'Y' ip_row = row ip_style = lv_style_right_guid ).

    column_dimension = lo_worksheet->get_column_dimension( ip_column = 'Z' ).
    column_dimension->set_visible( ip_visible = abap_false ).
  ENDLOOP.
...
*&---------------------------------------------------------------------*
*&      Form  F_MODIFY_STOCK_OPNAME
*&---------------------------------------------------------------------*
FORM f_modify_stock_opname  USING    fu_matnr.
  DATA: lv_qty   TYPE menge_d.

  FIELD-SYMBOLS: <fs_kemenkes1> LIKE gt_kemenkes,
                 <fs_kemenkes2> LIKE gt_kemenkes.

  READ TABLE gt_kemenkes ASSIGNING <fs_kemenkes1>
      WITH KEY matnr = fu_matnr
               in_code = '88'.

  READ TABLE gt_kemenkes ASSIGNING <fs_kemenkes2>
      WITH KEY matnr = fu_matnr
               out_code = '88'.

  IF <fs_kemenkes1> IS ASSIGNED.
    lv_qty = <fs_kemenkes1>-in_pbf.
    CLEAR: <fs_kemenkes1>-in_pbf,<fs_kemenkes1>-in_pbf_name.
  ENDIF.
  IF <fs_kemenkes2> IS ASSIGNED.
    lv_qty = lv_qty - <fs_kemenkes2>-out_pbf.
    CLEAR: <fs_kemenkes2>-out_pbf,<fs_kemenkes2>-out_pbf_name.
  ENDIF.

  IF lv_qty GT 0.
    IF <fs_kemenkes1> IS ASSIGNED.
      <fs_kemenkes1>-in_pbf = lv_qty.
      <fs_kemenkes1>-in_pbf_name = <fs_kemenkes1>-in_name1.
    ENDIF.
  ELSE.
    IF <fs_kemenkes2> IS ASSIGNED.
      <fs_kemenkes2>-out_pbf = abs( lv_qty ).
      <fs_kemenkes2>-out_pbf_name = <fs_kemenkes2>-out_name1.
    ENDIF.
  ENDIF.
ENDFORM.                    " F_MODIFY_STOCK_OPNAME

*&---------------------------------------------------------------------*
*&      Form  F_DELETE_INITIAL_ROW
*&---------------------------------------------------------------------*
FORM f_delete_initial_row  USING    fu_matnr.
  DELETE gt_kemenkes WHERE matnr      = fu_matnr AND
                           sak_jumlah = 0 AND
                           saw_jumlah = 0 AND
                           in_pabrik  = 0 AND
                           in_pbf     = 0 AND
