...
PARAMETERS : pa_bukrs LIKE reguh-zbukr DEFAULT '8190' MODIF ID hid,
             pa_laufd LIKE reguh-laufd OBLIGATORY,
             pa_laufi LIKE reguh-laufi OBLIGATORY.
SELECT-OPTIONS so_lifnr FOR reguh-lifnr OBLIGATORY NO INTERVALS NO-EXTENSION.
PARAMETERS : pa_bankn TYPE lfbk-bankn OBLIGATORY.
PARAMETERS : pa_banka TYPE bnka-banka MODIF ID pba.
PARAMETERS : pa_paydt TYPE dats DEFAULT sy-datum.   "NO-DISPLAY.
SELECTION-SCREEN SKIP 1.
PARAMETERS : pa_prev  AS CHECKBOX DEFAULT 'X'.
SELECTION-SCREEN END OF BLOCK block1.
SELECTION-SCREEN SKIP 1.
SELECTION-SCREEN BEGIN OF BLOCK c.
PARAMETERS : pa_name(45)  NO-DISPLAY, " AS LISTBOX VISIBLE LENGTH 45,
             pa_cekno(20) NO-DISPLAY,
             pa_acno(20)  NO-DISPLAY.
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR so_lifnr-low.
  PERFORM f_f4_vendor USING 'SO_LIFNR-LOW'.

AT SELECTION-SCREEN ON VALUE-REQUEST FOR pa_bankn.
  PERFORM f_f4_bank USING 'PA_BANKN'.
...
AT SELECTION-SCREEN OUTPUT.
  name = 'PA_NAME'.
  REFRESH: list.
  "data: lv_zbank type zfidt004-zbank, lv_acno type zfidt004-acno.
  SELECT zbank znorek INTO (value-key, value-text) FROM zfidt004
    WHERE bukrs = pa_bukrs.
    CONCATENATE value-key value-text INTO value-key SEPARATED BY ' '.
...
*      ENDIF.
*    ENDIF.
  ELSEIF t_header-zbukr EQ '8230' OR
    t_header-zbukr EQ '8190'.
  ...
  LOOP AT t_header.
    ...
    IF t_detail[] IS NOT INITIAL.
      ...
      ELSE.
        ...
        ls_ztusfidt001-bank_name  = pa_banka.  "pa_name.
        ls_ztusfidt001-bank_ac    = pa_bankn.  "pa_acno.
...
*        lt_acc    LIKE t_reguh OCCURS 0 WITH HEADER LINE.
  ...
  SELECT SINGLE lfbk~koinh lfbk~bankn
    bnka~banka bnka~brnch bnka~swift
    FROM lfbk JOIN bnka ON lfbk~banks = bnka~banks
                        AND lfbk~bankl = bnka~bankl
    INTO CORRESPONDING FIELDS OF gs_bank
    WHERE lfbk~lifnr = so_lifnr-low
      AND lfbk~bankn = pa_bankn.
...
*    t_header-name1  = t_reguh-name1.
    ...
    SPLIT pa_name AT space INTO  pa_name pa_acno.
...
*    ENDIF.
    ...
    IF gs_bank-banka IS INITIAL.
      t_header-banka = pa_banka.
      t_header-bankn = pa_bankn.
    ELSE.
      t_header-banka = gs_bank-banka.
      t_header-bankn = gs_bank-bankn.
    ENDIF.

    t_header-brnch = gs_bank-brnch.
    t_header-koinh = gs_bank-koinh.
    t_header-swift = gs_bank-swift.
...
*    t_header-zvoucher = 'PV'.
    ...
    PERFORM f_signature USING pa_bukrs t_reguh-lifnr t_header-total
                        CHANGING t_header-sign01 t_header-sign02
                                 t_header-sign03 t_header-sign04
                                 t_header-sign05.
      ...
      CASE t_regup-zbukr.
        ...
        WHEN OTHERS.
          IF t_regup-zbukr = '8020' OR t_regup-zbukr = '8070'.
            t_vdata-zfbdt  = t_bsik-zfbdt.
          ELSEIF t_regup-zbukr = '8190'.
            READ TABLE gt_bkpf INTO ls_bkpf
                               WITH KEY belnr = t_regup-belnr
                                        bukrs = t_regup-zbukr
                                        gjahr = t_regup-gjahr.
            IF sy-subrc = 0.
              t_vdata-zfbdt  = ls_bkpf-bldat + t_bsik-zbd1t.
            ENDIF.
          ELSE.
...
*&      Form  F_F4_BANK
*&---------------------------------------------------------------------*
FORM f_f4_bank  USING    fu_field.
  TYPES : BEGIN OF ty_f4bank,
            bankn TYPE lfbk-bankn,
            koinh TYPE lfbk-koinh,
            banka TYPE bnka-banka,
            swift TYPE bnka-swift,
  ...
  DATA : lt_bank    TYPE STANDARD TABLE OF ty_f4bank,
  ...
  PERFORM f_dynp_value_read USING 'SO_LIFNR-LOW'
                            CHANGING lv_lifnr.
  ...
  IF lv_lifnr IS NOT INITIAL.
    ...
    SELECT lfbk~bankn lfbk~koinh bnka~banka bnka~swift
      FROM lfbk JOIN bnka ON lfbk~banks = bnka~banks
                          AND lfbk~bankl = bnka~bankl
      INTO CORRESPONDING FIELDS OF TABLE lt_bank
      WHERE lfbk~lifnr = lv_lifnr.
    ...
    IF lt_bank[] IS NOT INITIAL.
      ...
      PERFORM f_value_request TABLES return_tab
                              USING 'BANKN' fu_field
                              CHANGING lv_subrc.
      IF lv_subrc = 0.
        READ TABLE return_tab INTO ls_return INDEX 1.
        IF sy-subrc = 0.
          lv_bankn  = ls_return-fieldval.
          READ TABLE lt_bank INTO ls_bank WITH KEY bankn = lv_bankn.
          IF sy-subrc = 0.
            PERFORM f_dynpfield TABLES dynpfields
                                USING fu_field ls_bank-bankn ''.
            PERFORM f_dynpfield TABLES dynpfields
                                USING 'PA_BANKA' ls_bank-banka ''.
          ENDIF.
        ENDIF.
...
*&      Form  F_F4_VENDOR
*&---------------------------------------------------------------------*
FORM f_f4_vendor  USING    fu_field.
  ...
  DATA : lv_laufd TYPE reguh-laufd,
         lv_laufi TYPE reguh-laufi,
         lv_zbukr TYPE reguh-zbukr,
         lv_subrc TYPE sy-subrc.

  DATA : lr_vblnr TYPE RANGE OF vblnr,
         ls_vblnr LIKE LINE OF lr_vblnr.

  PERFORM f_dynp_value_read USING 'PA_LAUFD'
                            CHANGING lv_laufd.
  PERFORM f_dynp_value_read USING 'PA_LAUFI'
                            CHANGING lv_laufi.
  ...
  ls_vblnr-sign   = 'I'.
  ls_vblnr-option = 'CP'.
  APPEND ls_vblnr TO lr_vblnr.

  IF lv_laufd IS NOT INITIAL AND
    lv_laufi IS NOT INITIAL AND
    lv_zbukr IS NOT INITIAL AND
    lr_vblnr[] IS NOT INITIAL.
    SELECT *
      FROM reguh
      ...
      WHERE laufd = lv_laufd
        AND laufi = lv_laufi
        AND zbukr = lv_zbukr
        AND vblnr IN lr_vblnr.

    SORT lt_reguh BY lifnr.
    DELETE ADJACENT DUPLICATES FROM lt_reguh COMPARING lifnr.
    LOOP AT lt_reguh INTO ls_reguh.
      ls_vendor-lifnr = ls_reguh-lifnr.
      ls_vendor-name1 = ls_reguh-name1.
      APPEND ls_vendor TO lt_vendor.
      CLEAR ls_vendor.
    ENDLOOP.
    ASSIGN lt_vendor[] TO <fs_tab>.
    IF lt_vendor[] IS NOT INITIAL.
      CLEAR lv_subrc.
      PERFORM f_value_request TABLES return_tab
                              USING 'LIFNR' fu_field
                              CHANGING lv_subrc.
    ENDIF.
  ENDIF.
ENDFORM.
...
FORM f_signature  USING    fu_bukrs fu_lifnr fu_wrbtr
  ...
  DATA : ls_014   LIKE LINE OF gt_014,
  ...
  CLEAR : fc_sign1, fc_sign2, fc_sign3, fc_sign4.
...
*&---------------------------------------------------------------------*
FORM f_selection_screen .
  IF so_lifnr-low = '0800001286'.

  ENDIF.
ENDFORM.

*&---------------------------------------------------------------------*
*&      Form  F_ADDITIONAL_BANK
*&---------------------------------------------------------------------*
FORM f_additional_bank .
  DATA : ls_014   LIKE LINE OF gt_014.
  ...
  READ TABLE gt_014 INTO ls_014
                    WITH KEY bukrs = pa_bukrs
                             field = 'LIFNR_ZFIF009'.
  IF sy-subrc = 0.
    IF so_lifnr-low = ls_014-value.
      gv_subrc = 4.
