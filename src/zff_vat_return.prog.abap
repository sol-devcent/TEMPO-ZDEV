...
*&                                            Project                  *
************************************************************************
REPORT zff_vat_return NO STANDARD PAGE HEADING
                      MESSAGE-ID zf
                      LINE-SIZE 121.
...
DATA: gs_header   TYPE zfist001,
      ...
      answer,
      icon1   TYPE icon-name,
      icon2   TYPE icon-name.
...
PARAMETERS:
  ...
  p_belnr LIKE bsis-belnr OBLIGATORY MODIF ID pbe.

SELECT-OPTIONS:
 s_belnr  FOR bsis-belnr MODIF ID sbe.

PARAMETERS:
  p_gjahr LIKE bsis-gjahr OBLIGATORY DEFAULT sy-datum+0(4),
* Pa_monat like bsis-monat default sy-datum+4(2) obligatory.

    sign(24) OBLIGATORY MODIF ID sig.
...
PARAMETERS check1 AS CHECKBOX MODIF ID td1.
SELECTION-SCREEN COMMENT 41(20) text-008 FOR FIELD check1 MODIF ID td1.
SELECTION-SCREEN END OF LINE.
...
SELECTION-SCREEN POSITION 40.
PARAMETERS check2 AS CHECKBOX MODIF ID td2.
SELECTION-SCREEN COMMENT 41(20) text-008 FOR FIELD check2 MODIF ID td2.
SELECTION-SCREEN END OF LINE.
...
START-OF-SELECTION.
  ...
  IF p_type1 = 'X'.
    PERFORM get_header_data CHANGING l_stblg.
    IF sy-subrc = 0.
      IF l_stblg IS NOT INITIAL.
        CONCATENATE 'Doc.' p_belnr 'has been cancelled with' l_stblg
        INTO l_mess
        SEPARATED BY space.
        MESSAGE s000(zf) WITH l_mess DISPLAY LIKE 'E'.
      ELSE.
  ...
  IF p_type2 = 'X'.
    ...
    IF sy-subrc = 0.
      ...
      MOVE p_bukrs TO va_bukrs.
      MOVE p_belnr TO va_belnr.
      MOVE p_gjahr TO va_gjahr.
      PERFORM cek_print.
*      IF SY-SUBRC NE 0.
*        PERFORM f_popup using VA_BUKRS
*                              VA_BELNR
*                              VA_GJAHR.
*      ENDIF.
      ...
      PERFORM cetak_form.
      va_belnr = p_belnr.
      PERFORM print_update USING p_belnr.
      ADD 1 TO page1.
...
*********      ELSE.
      icon1 = 'ICON_PRINT'.
      icon2 = 'ICON_LAYOUT_CONTROL'.

      CALL FUNCTION 'POPUP_TO_CONFIRM'
        EXPORTING
          titlebar      = 'Print'
          text_question = 'Print/Print Preview'
          text_button_1 = 'Print'
          icon_button_1 = icon1
          text_button_2 = 'Print preview'
          icon_button_2 = icon2
        IMPORTING
          answer        = answer.
      ...
      CASE answer.
        WHEN 'A'.
        WHEN OTHERS.
          PERFORM f_cetak_form_new USING answer.

          va_belnr = p_belnr.
          IF sy-ucomm = 'PRNT'.
            PERFORM f_insert_to_table.
            PERFORM f_change_fi_document.
          ENDIF.
          PERFORM print_update USING p_belnr.
          ADD 1 TO page1.
      ENDCASE.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM get_header_data CHANGING fc_stblg.
...
FORM cetak_form.
  ...
  CLEAR: wa_itab1.
  SORT i_itab1 BY rbeln DESCENDING.
  LOOP AT i_itab1 INTO wa_itab1.
    ...
    CONCATENATE p_bukrs p_belnr p_gjahr INTO l_fname.
...
*    ENDIF.
    ...
    CLEAR: wa_itab1, va_lines, counter1.
  ENDLOOP.
ENDFORM.                    " CETAK_FORM
...
MODULE user_command_0900 INPUT.
  ...
  CLEAR: wa_itab3, i_itab3.

  CASE sy-ucomm.
    WHEN 'CONT'.
      IF text1 EQ space AND
         tgl1  EQ 0.
        MESSAGE i000(zf) WITH 'Invalid Entry in Pop-Up Screen'.
      ELSE.
        WRITE tgl1 TO l_tgl DD/MM/YYYY.
        CONCATENATE text1 '/' l_tgl INTO wa_itab3-tdline
          SEPARATED BY space.
        MOVE '01' TO  wa_itab3-tdformat.
        APPEND wa_itab3 TO i_itab3.
        CLEAR l_tgl.

        IF text2 NE space AND
           tgl2  NE 0.
          WRITE tgl2 TO l_tgl DD/MM/YYYY.
          CONCATENATE text2 '/' l_tgl INTO wa_itab3-tdline
            SEPARATED BY space.
          MOVE '02' TO  wa_itab3-tdformat.
          APPEND wa_itab3 TO i_itab3.
          CLEAR l_tgl.
        ENDIF.

        IF text3 NE space AND
           tgl3  NE 0.
          WRITE tgl3 TO l_tgl DD/MM/YYYY.
          CONCATENATE text3 '/' l_tgl INTO wa_itab3-tdline
            SEPARATED BY space.
          MOVE '03' TO wa_itab3-tdformat.
          APPEND wa_itab3 TO i_itab3.
          CLEAR l_tgl.
        ENDIF.

        IF text4 NE space AND
           tgl4  NE 0.
          WRITE tgl4 TO l_tgl DD/MM/YYYY.
          CONCATENATE text4 '/' l_tgl INTO wa_itab3-tdline
            SEPARATED BY space.
          MOVE '04' TO wa_itab3-tdformat.
          APPEND wa_itab3 TO i_itab3.
          CLEAR l_tgl.
        ENDIF.

        IF text5 NE space AND
           tgl5  NE 0.
          WRITE tgl5 TO l_tgl DD/MM/YYYY.
          CONCATENATE text5 '/' l_tgl INTO wa_itab3-tdline
            SEPARATED BY space.
          MOVE '05' TO wa_itab3-tdformat.
          APPEND wa_itab3 TO i_itab3.
          CLEAR l_tgl.
        ENDIF.
        LEAVE TO SCREEN 0.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM f_popup USING p_bukrs LIKE bkpf-bukrs
                   p_belnr LIKE bkpf-belnr
                   p_gjahr LIKE bkpf-gjahr.

  DATA: l_fname(30), vheader LIKE thead.

  CLEAR: text1, text2, text3, text4, text5,
         tgl1, tgl2, tgl3, tgl4, tgl5.

  CALL SCREEN 900 STARTING AT 25  6
                  ENDING   AT 56 13.

  CLEAR: l_fname.

  CONCATENATE p_bukrs p_belnr p_gjahr  INTO l_fname.
  vheader-tdobject = 'BELEG'.
  vheader-tdname   = l_fname.
  vheader-tdid     = '0004'.
  vheader-tdspras  = 'E'.
  ...
  IF sy-subrc NE 0.
    MESSAGE i000(zf) WITH 'Error Write Text data'.
  ENDIF.
