...
FORM f_get_data.
  ...
  DATA: lt_zfgscab   LIKE gt_zfgscab OCCURS 0 WITH HEADER LINE,
        ...
        lt_zfgsdnstm LIKE gt_zfgsdnstm OCCURS 0 WITH HEADER LINE.
...
*          WHERE vbund EQ lt_kna1-vbund.
    ...
    WHEN radio2.
      SELECT bukrs gsber belnr gjahr nomordn belnrdn belnrdnrev bktxt
        ...
        FROM zfgsdnstm
        INTO CORRESPONDING FIELDS OF TABLE gt_zfgsdnstm
        WHERE bukrs   EQ pa_bukrs
          AND nomordn IN so_nodn
          AND flag    EQ space.

      lt_zfgsdnstm[] = gt_zfgsdnstm[].
      SORT lt_zfgsdnstm BY bukrs gsber belnr gjahr.
      DELETE ADJACENT DUPLICATES FROM lt_zfgsdnstm COMPARING bukrs gsber belnr gjahr.
      IF lt_zfgsdnstm[] IS NOT INITIAL.
        SELECT bukrs gsber belnr gjahr buzei budat bldat xblnr xref2 xref3 zuonr
               sgtxt zgsno ztype zsubtype vbund kunnr waers shkzg wrbtr
               hkont txt1 txt2 txt3 txt4 belnrgs belnrpost belnrdn gjahrpost
               userpost tglpost jampost belnrrev belnrrevdn userrev tglrev
          FROM zfgscab
          INTO TABLE gt_zfgscab
          FOR ALL ENTRIES IN lt_zfgsdnstm
          WHERE bukrs      EQ lt_zfgsdnstm-bukrs
            AND gsber      EQ lt_zfgsdnstm-gsber
            AND belnr      EQ lt_zfgsdnstm-belnr
            AND gjahr      EQ lt_zfgsdnstm-gjahr.
      ENDIF.

      lt_zfgsdnstm[] = gt_zfgsdnstm[].
      SORT lt_zfgsdnstm BY kunnr.
      DELETE ADJACENT DUPLICATES FROM lt_zfgsdnstm COMPARING kunnr.
      IF lt_zfgsdnstm[] IS NOT INITIAL.
        SELECT a~kunnr b~name1 street post_code1 city1
          FROM kna1 AS a JOIN adrc AS b ON a~adrnr EQ b~addrnumber
          INTO TABLE gt_kna1
          FOR ALL ENTRIES IN lt_zfgsdnstm
          WHERE a~kunnr EQ lt_zfgsdnstm-kunnr.
      ENDIF.
...
*          WHERE vbund EQ lt_kna11-vbund.
    ...
    WHEN radio5.
      ...
      CHECK gt_zfgsdnstm[] IS NOT INITIAL.

      SELECT bukrs belnr gjahr buzei bschl koart shkzg dmbtr zuonr hkont
        FROM bseg
        APPENDING TABLE gt_bseg
        FOR ALL ENTRIES IN gt_zfgsdnstm
        WHERE bukrs EQ gt_zfgsdnstm-bukrs
          AND gjahr EQ gt_zfgsdnstm-gjahr
          AND belnr EQ gt_zfgsdnstm-belnrdn
...
FORM f_build_fieldcat TABLES ft_report
  ...
  CASE 'X'.
    ...
    WHEN radio2 OR radio3.
      PERFORM f_fieldcatg USING ft_report:
        'BUKRS' 'ZFGSDNSTM' 'BUKRS' '' '' '' '' '' '' '' '' '' '' '' '' '',
        'GJAHR' 'ZFGSDNSTM' 'GJAHR' '' '' '' '' '' '' '' '' '' '' '' '' '',
        'BELNR' 'ZFGSDNSTM' 'BELNR' '' '15' 'Doc.Post KP' '' '' '' '' '' '' '' '' '' '',
        'NOMORDN' 'ZFGSDNSTM' 'NOMORDN' '' '' '' '' '' '' '' '' '' '' '' '' '',
        'BELNRDN' 'ZFGSDNSTM' 'BELNRDN' '' '15' 'Doc.Post DN' '' 'X' '' '' ''
        '' '' '' '' ''.
    ...
    WHEN radio5.
      PERFORM f_fieldcatg USING ft_report:
        ...
        'NOMORDN' 'ZFGSDNSTM' 'NOMORDN' '' '' 'No.DN/kuitansi' '' '' '' '' '' '' '' '' '' '',
        ...
        '' '' '' '' '' '' '' '',
        'BELNRDNREV' 'ZFGSDNSTM' 'BELNRDNREV' '' '12' 'No.Rev.Doc.DN' '' ''
        '' '' '' '' '' '' '' ''.
  ENDCASE.
ENDFORM.                    " F_FIELDCAT
...
FORM f_process_data.
  ...
  CASE 'X'.
    ...
    WHEN radio2 OR radio3.
      LOOP AT gt_zfgsdnstm.
        MOVE-CORRESPONDING gt_zfgsdnstm TO gt_out.
    ...
    WHEN radio4.
      ...
      LOOP AT gt_bseg WHERE shkzg EQ 'S'.
        READ TABLE gt_zfgsdnstm WITH KEY belnr = gt_bseg-belnr
                                         gjahr = gt_bseg-gjahr.
        IF sy-subrc EQ 0.
          CONTINUE.
        ELSE.
    ...
    WHEN radio5.
      LOOP AT gt_zfgscab.
        ...
        READ TABLE gt_zfgsdnstm WITH KEY belnrdn = gt_zfgscab-belnrdn.
        IF sy-subrc EQ 0.
          READ TABLE gt_bseg WITH KEY belnr = gt_zfgsdnstm-belnrdn
                                      gjahr = gt_zfgsdnstm-gjahr.
          IF sy-subrc EQ 0.
            ...
            ENDIF.
          ENDIF.
          gt_out-budatdn      = gt_zfgsdnstm-budat.
          gt_out-nomordn      = gt_zfgsdnstm-nomordn.
          gt_out-xblnr        = gt_zfgsdnstm-xblnr.
          gt_out-bktxt        = gt_zfgsdnstm-bktxt.
          gt_out-xref2        = gt_zfgsdnstm-xref2.
          gt_out-maktx        = gt_zfgsdnstm-maktx.
          gt_out-wrbtrdn      = gt_zfgsdnstm-wrbtr.
          gt_out-nopaaf       = gt_zfgsdnstm-nopaaf.
          gt_out-belnrdnrev   = gt_zfgsdnstm-belnrdnrev.
        ENDIF.
        ...
        WHEN radio1 OR radio4.
          CASE ffield.
            WHEN 'GT_OUT-ZSUBTYPE'.
              ...
              gt_out-kunnrx   = gt_out-kunnr.
              gt_out-vbundx   = gt_out-vbund.
              ...
              IF sy-subrc EQ 0.
                IF gt_subtype-loekz IS INITIAL.
                  IF pa_prev IS INITIAL.
                    UPDATE zfgscab SET vbund  = gt_out-vbund
                                       kunnr  = gt_out-kunnr
                                       vbundx = gt_out-vbundx
                                       kunnrx = gt_out-kunnrx
                                   WHERE bukrs = gt_out-bukrs
                                     AND gsber = gt_out-gsber
                                     AND belnr = gt_out-belnr
                                     AND gjahr = gt_out-gjahr
                                     AND buzei = gt_out-buzei.
                  ENDIF.
      ...
      IF pa_ztype IS NOT INITIAL AND
        ...
        IF lv_stm IS INITIAL.
          LOOP AT SCREEN.
            ...
            MODIFY SCREEN.
          ENDLOOP.
          MESSAGE e000(zab) WITH 'Error in Type or SubType - STM only'.
        ENDIF.
      ENDIF.
  ...
  IF fu_belnr1 IS NOT INITIAL.
    ...
    ELSE.
      ...
      IF sy-subrc = 0.
        ...
        UPDATE zfgsdnstm SET   belnrdnrev = t_bdcmsg-msgv1
                                flag       = 'X'
                                zuserrev   = sy-uname
                                zdatumrev  = sy-datum
                          WHERE belnr  EQ fu_belnr
...
*                                    lv_gjahr pa_budat.
  ...
  PERFORM f_save_zfgsdnstm USING    fwa_out lv_belnr '0'
                            CHANGING fc_subrc.
...
FORM f_print_dn USING p_formname TYPE tdsfname fu_reprint
  ...
  DATA : ls_out     LIKE LINE OF gt_out,
         ls_zfgscab LIKE LINE OF gt_zfgscab.
  ...
  LOOP AT gt_post.
    IF gt_post-bschl EQ '01'.
      ...
      ENDIF.
      CLEAR ls_zfgscab.
      READ TABLE gt_zfgscab INTO ls_zfgscab
                            WITH KEY bukrs = gt_post-bukrs
                                     gsber = gt_post-gsber
                                     belnr = gt_post-belnr
                                     gjahr = gt_post-gjahr.
      IF sy-subrc = 0.
        gt_head-zgsno  = ls_zfgscab-zgsno.
      ENDIF.
      COLLECT gt_head.
      CLEAR lv_kunnr.
    ENDIF.
    ...
    IF sy-subrc EQ 0.
      ...
      CASE 'X'.
        WHEN radio1 OR radio4.
          ...
          CONCATENATE gt_head-nomordn '/STM/' lv_period '/' lv_nomor
...
*&---------------------------------------------------------------------*
*&      Form  F_SAVE_ZFGSDNSTM
*&---------------------------------------------------------------------*
FORM f_save_zfgsdnstm  USING    fwa_out STRUCTURE gt_out
                                 fu_belnr fu_proc
                        CHANGING fc_subrc.

  DATA: lt_stm   LIKE zfgsdnstm OCCURS 0 WITH HEADER LINE.

  READ TABLE gt_post WITH KEY belnr = fwa_out-belnr.
  IF sy-subrc EQ 0.
    lt_stm-nomordn       = gt_post-nomordn.
  ENDIF.

  lt_stm-bukrs         = fwa_out-bukrs.
  lt_stm-gsber         = fwa_out-gsber.
  lt_stm-belnr         = fwa_out-belnr.
  lt_stm-gjahr         = fwa_out-gjahr.
  lt_stm-belnrdn       = fu_belnr.
  lt_stm-bktxt         = fwa_out-bktxt.
  lt_stm-xblnr         = fwa_out-xblnr.
  lt_stm-nopaaf        = fwa_out-nopaaf.
  lt_stm-maktx         = fwa_out-maktx.
  lt_stm-xref2         = fwa_out-xref2.
  lt_stm-xref3         = fwa_out-xref3.
  lt_stm-waers         = 'IDR'.
  lt_stm-wrbtr         = fwa_out-wrbtr.
  lt_stm-kunnr         = fwa_out-kunnr.
  lt_stm-budat         = fwa_out-budat.
  lt_stm-zuser         = sy-uname.
  lt_stm-zdatum        = sy-datum.
  lt_stm-zuzeit        = sy-uzeit.
  APPEND lt_stm.

  CASE fu_proc.
    WHEN '0'.
      INSERT zfgsdnstm FROM TABLE lt_stm.
    WHEN OTHERS.
  ENDCASE.
ENDFORM.                    " F_SAVE_ZFGSDNSTM
