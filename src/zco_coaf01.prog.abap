...
FORM f_get_data .
  ...
  SELECT aufnr plnbez ktext gamng waers objnr gltri gstri
    FROM caufv
  ...
  IF gt_caufv[] IS NOT INITIAL.
    ...
    SELECT aufnr wemng wewrt matnr
      FROM afpo
...
FORM f_process_data .
  ...
  LOOP AT gt_cosp INTO ls_cosp.
    ...
    IF sy-subrc = 0.
      ...
      ls_detail-plnbez    = ls_caufv-plnbez.
      ls_detail-ktext     = ls_caufv-ktext.
      ls_detail-gltri     = ls_caufv-gltri.
      ls_detail-gstri     = ls_caufv-gstri.
    ENDIF.
  ...
  IF sy-subrc = 0.
    gs_header-plnbez = ls_material-matnr.
  ENDIF.
  ...
  CASE fu_container.
    ...
    WHEN 'ALV'.
      PERFORM f_fieldcatg USING 'GT_ALV' :
        'PLNBEZ' 'CAUFV' 'PLNBEZ' '' '12' '' '' '' '' '' '' '' '' '' ''
        '' '' '' '' '' 'X',
        'KTEXT' 'CAUFV' 'KTEXT' '' '30' '' '' '' '' '' '' '' '' '' ''
        '' '' '' '' '' 'X',
        'GSTRI' 'CAUFV' 'GSTRI' '' '12' '' '' '' '' '' '' '' '' '' ''
        '' '' '' '' '' 'X',
        'GLTRI' 'CAUFV' 'GLTRI' '' '12' '' '' '' '' '' '' '' '' '' ''
        '' '' '' '' '' 'X',
...
FORM f_create_hierarchy .
  ...
  LOOP AT gt_material INTO ls_material.
    ...
    lv_node = ls_material-matnr.
    wa_tree-node_main = ls_material-matnr.
    PERFORM f_add_node_main USING    wa_tree '' lv_node
                            CHANGING lv_key_main.

    LOOP AT gt_caufv INTO ls_caufv WHERE plnbez = ls_material-matnr.
      CLEAR lv_node.
  ...
  CASE fu_flag.
    WHEN '1'.
      ...
      READ TABLE gt_caufv INTO ls_caufv
                          WITH KEY plnbez = gs_header-plnbez.
      IF sy-subrc = 0.
        ...
        LOOP AT gt_afpo INTO ls_afpo WHERE matnr = gs_header-plnbez.
...
FORM f_check_status_order .
  ...
  LOOP AT gt_caufv INTO ls_caufv.
    ...
    ls_material-matnr = ls_caufv-plnbez.
...
FORM f_detail_to_alv  TABLES   ft_alv STRUCTURE gt_detlgen
                      USING    fs_detail  STRUCTURE gt_detlgen.
  DATA : ls_alv       TYPE ty_detail,
         ls_resb      TYPE resb,
         ls_makt      TYPE makt,
  ...
  IF fs_detail-matnr IS NOT INITIAL.
    APPEND ls_alv TO ft_alv.
  ELSE.
    READ TABLE gt_resb INTO ls_resb WITH KEY aufnr = fs_detail-aufnr
                                             saknr = fs_detail-kstar
                                    TRANSPORTING NO FIELDS.
    IF sy-subrc IS INITIAL.
      LOOP AT gt_resb INTO ls_resb WHERE aufnr = fs_detail-aufnr
                                     AND saknr = fs_detail-kstar.
