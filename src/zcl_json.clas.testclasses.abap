...
*
*----------------------------------------------------------------------*
CLASS lc_json_custom DEFINITION FINAL INHERITING FROM zcl_json.
  PUBLIC SECTION.
    CLASS-METHODS:
      class_constructor,
...
CLASS lc_json_custom IMPLEMENTATION.
  ...
  METHOD class_constructor.
    CONCATENATE mc_bool_types `\TYPE=zBOOLEAN` INTO mc_bool_types.
...
*----------------------------------------------------------------------*
*       CLASS lcl_dyn_access DEFINITION
*----------------------------------------------------------------------*
*
*----------------------------------------------------------------------*
CLASS lcl_dyn_access DEFINITION.
  ...
  PUBLIC SECTION.
    CLASS-METHODS:
      create IMPORTING ir_data TYPE REF TO data OPTIONAL
                       iv_data TYPE data OPTIONAL
                       iv_component TYPE string OPTIONAL
             RETURNING value(ro_ref) TYPE REF TO lcl_dyn_access.

    METHODS:
      constructor IMPORTING ir_data TYPE REF TO data OPTIONAL
                            iv_data TYPE data OPTIONAL,
      at  IMPORTING iv_component TYPE string OPTIONAL
          RETURNING value(ro_ref) TYPE REF TO lcl_dyn_access,
      empty RETURNING value(rv_val) TYPE abap_bool,
      ref RETURNING value(rv_data) TYPE REF TO data,
      value EXPORTING ev_data TYPE data.

  PROTECTED SECTION.
    DATA: mr_data TYPE REF TO data.

    CLASS-METHODS:
      deref  IMPORTING ir_data TYPE REF TO data
             RETURNING value(rr_data) TYPE REF TO data.

    METHODS:
      at_int IMPORTING iv_component TYPE string OPTIONAL
                       iv_index     TYPE i OPTIONAL
                       iv_keys      TYPE string OPTIONAL
             RETURNING value(ro_ref) TYPE REF TO lcl_dyn_access.

ENDCLASS.                    "lcl_dyn_access DEFINITION

*----------------------------------------------------------------------*
*       CLASS lcl_dyn_access IMPLEMENTATION
*----------------------------------------------------------------------*
*
*----------------------------------------------------------------------*
CLASS lcl_dyn_access IMPLEMENTATION.
      ...
      ELSEIF iv_keys IS NOT INITIAL.
        ...
        IF sy-subrc IS INITIAL.
          ...
          ELSE.
            <data> = lv_key.
          ENDIF.
          READ TABLE <table> FROM <data> REFERENCE INTO <ref> . "#EC CI_ANYSEQ
          IF sy-subrc IS NOT INITIAL.
            UNASSIGN <ref>.
          ENDIF.
        ELSE.
...
ENDCLASS.                    "lcl_dyn_access IMPLEMENTATION
...
CLASS abap_unit_testclass IMPLEMENTATION.
  ...
  METHOD serialize_numbers.
    ...
    lv_act = zcl_json=>serialize( data = ls_data numc_as_string = abap_true ).
  ...
  METHOD serialize_booleans.
    ...
    TYPES:
      BEGIN OF t_data,
            ...
            x_bool_true         TYPE zcl_json=>bool,
            x_bool_false        TYPE zcl_json=>bool,
            x_tribool_true      TYPE zcl_json=>tribool,
            x_tribool_false     TYPE zcl_json=>tribool,
            x_tribool_undefined TYPE zcl_json=>tribool,
        END OF t_data.
    ...
    ls_data-tribool_true        = zcl_json=>c_tribool-true.
    ls_data-tribool_false       = zcl_json=>c_tribool-false.
    ls_data-tribool_undefined   = zcl_json=>c_tribool-undefined.
    ls_data-x_bool_true         = zcl_json=>c_bool-true.
    ls_data-x_bool_false        = zcl_json=>c_bool-false.
    ls_data-x_tribool_true      = zcl_json=>c_tribool-true.
    ls_data-x_tribool_false     = zcl_json=>c_tribool-false.
    ls_data-x_tribool_undefined = zcl_json=>c_tribool-undefined.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_act CHANGING data = ls_act ).
    ...
    lv_act = zcl_json=>serialize( data = ls_data compress = abap_true ).
  ...
  METHOD serialize_types.
    ...
    CONCATENATE `{"FLAG":true,"CHAR":"\"TEST\\\"","NUMC":12345678,"STRING":"ABCDEFG","XSTRING":"q83v","INTEGER":42,"FLOAT":3.1415926535897900E+00,`
                `"PACKED":3.141593,"HEX":"AAAAAAAAOt5osQ==","TSL":20151002134450.5545900,"TS":20150219142259,"DATE":"2015-02-19","TIME":"15:22:59"}` INTO lv_exp.
    lv_act    = zcl_json=>serialize( data = ls_data ).
    ...
    CONCATENATE `{"FLAG":true,"CHAR":"\"TEST\\\"","NUMC":12345678,"STRING":"ABCDEFG","XSTRING":"q83v","INTEGER":42,"FLOAT":3.1415926535897900E+00,`
                `"PACKED":3.141593,"HEX":"AAAAAAAAOt5osQ==","TSL":"2015-10-02T13:44:50.5545900Z","TS":"2015-02-19T14:22:59.0000000Z","DATE":"2015-02-19","TIME":"15:22:59"}` INTO lv_exp.
    lv_act    = zcl_json=>serialize( ts_as_iso8601 = abap_true data = ls_data ).
  ...
  METHOD serialize_associative_array.
    ...
    lv_act = zcl_json=>serialize( data = lt_table1 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).

    cl_aunit_assert=>assert_equals( act = lv_act exp = lv_exp msg = 'Serialization of standard fails' ).

    lv_exp = '[{"key1":"k100","key2":0,"value1":"v100"},{"key1":"k101","key2":1,"value1":"v101"},{"key1":"k102","key2":2,"value1":"v102"}]'.
    lv_act = zcl_json=>serialize( data = lt_table2 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).
    ...
    lv_act = zcl_json=>serialize( data = lt_table3 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).

    cl_aunit_assert=>assert_equals( act = lv_act exp = lv_exp msg = 'Serialization of sorted table with unique fails' ).

    lv_exp = '{"k100":{"key2":0,"value1":"v100"},"k101":{"key2":1,"value1":"v101"},"k102":{"key2":2,"value1":"v102"}}'.
    lv_act = zcl_json=>serialize( data = lt_table4 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).
    ...
    lv_act = zcl_json=>serialize( data = lt_table5 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).
    ...
    lv_act = zcl_json=>serialize( data = lt_table6 pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true ).
  ...
  METHOD serialize_table.
    ...
    ls_data-ff-tablet   = zcl_json=>c_tribool-true.
    ls_data-ff-phone    = abap_false.
    ls_data-desktop     = abap_true.
    ls_data-tablet      = zcl_json=>c_tribool-false.
...
*                           RESULT XML xml.                  "#EC NOTEXT
  ...
  METHOD serialize_upper_camel_case.
    ...
    ls_data-__underscore      = abap_true.
    ls_data-_upper_camel_case = abap_false.
    ls_data-camel_case        = abap_true.
    ls_data-lowcase           = abap_true.

    lv_exp = '{"_underscore":true,"under_score":false,"UpperCamelCase":false,"camelCase":true,"lowcase":true}'.
    lv_act = zcl_json=>serialize( data = ls_data pretty_name = zcl_json=>pretty_mode-camel_case ).
  ...
  METHOD deserialize_camel_case.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = ls_act ).
  ...
  METHOD serialize_included_types.
    ...
    lv_exp = '{"objects":[{"objtype":"","objname":"","issues":[],"includes":[],"path":"","source":"","sourceLength":0}]}'.
    lv_act = zcl_json=>serialize( data = ls_data pretty_name = zcl_json=>pretty_mode-camel_case ).
  ...
  METHOD deserialize_non_fitting_table.
    ...
    zcl_json=>deserialize( EXPORTING json = json CHANGING data = itab ).
...
**********************************************************************
  ...
  METHOD deserialize_types.
    ...
    lv_data = '{"negative_i": -1, "positive_i":10000, "positive_n" : "000001", "boolean" : true, "timestamp" : 1419279663821, "timestamp_not_mapped" : 1419279663821}'.
    zcl_json=>deserialize( EXPORTING json = lv_data pretty_name = zcl_json=>pretty_mode-low_case CHANGING data = lv_act ).
...
**********************************************************************
  ...
  METHOD deserialize_associative_array.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_data assoc_arrays = abap_true CHANGING data = lt_act ).
  ...
  METHOD deserialize_empty_structure.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_data pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = lt_act ).
  ...
  METHOD serialize_object.
    ...
    lv_act = zcl_json=>serialize( data = lo_data ).
  ...
  METHOD deserialize_object.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_data CHANGING data =  lo_data ).
  ...
  METHOD deserialize_partial.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json assoc_arrays = abap_true pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = lt_act ).
  ...
  METHOD serialize_partial.
    ...
    DATA: lv_exp   TYPE zcl_json=>json,
          lv_act   TYPE zcl_json=>json,
          lt_act   TYPE SORTED TABLE OF ts_record WITH UNIQUE KEY id,
          ls_act   LIKE LINE OF lt_act.
    ...
    lv_act = zcl_json=>serialize( data = lt_act assoc_arrays = abap_true pretty_name = zcl_json=>pretty_mode-camel_case ).
  ...
  METHOD deserialize_alias_type.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case
  ...
  METHOD name_value_map.
    ...
    lv_act = zcl_json=>serialize( data = lt_exp assoc_arrays = abap_true assoc_arrays_opt = abap_true ).
    cl_aunit_assert=>assert_equals( act = lv_act exp = lv_exp msg = 'Name/Value map serialization fails!' ).

    zcl_json=>deserialize( EXPORTING json = lv_act assoc_arrays = abap_true assoc_arrays_opt = abap_true
    ...
    lv_act = zcl_json=>serialize( data = lt_exp2 assoc_arrays = abap_true assoc_arrays_opt = abap_true ).
    cl_aunit_assert=>assert_equals( act = lv_act exp = lv_exp msg = 'Name/Value map serialization fails!' ).
  ...
  METHOD dynamic_types.
    ...
    ls_exp-data      = zcl_json=>serialize( data = lt_flight compress = abap_true pretty_name = zcl_json=>pretty_mode-camel_case ).
    lo_typedescr     = cl_abap_typedescr=>describe_by_data( lt_flight ).
    ls_exp-abap_type = lo_typedescr->absolute_name.
    lv_json          = zcl_json=>serialize( data = ls_exp compress = abap_true pretty_name = zcl_json=>pretty_mode-camel_case ).
    ...
    zcl_json=>deserialize( EXPORTING json = ls_act-data pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = <data> ).
  ...
  METHOD serialize_dynamic_type.
    ...
    lv_act = zcl_json=>serialize( data = lr_table pretty_name = zcl_json=>pretty_mode-camel_case ).
  ...
  METHOD deserialze_to_read_only.
    ...
    lv_json = zcl_json=>serialize( lo_exp ).
    zcl_json=>deserialize( EXPORTING json = lv_json CHANGING data = lo_exp ).
  ...
  METHOD generate.
    ...
    DATA: lv_json   TYPE zcl_json=>json,
          ...
          lo_data   TYPE REF TO lcl_dyn_access,
    ...
    lr_act = zcl_json=>generate( json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case ).
    cl_aunit_assert=>assert_not_initial( act = lr_act msg = 'Generation of ABAP object fails!' ).

    lo_data = lcl_dyn_access=>create( ir_data = lr_act iv_component = 'output-additional_properties').
    ...
    lo_data = lcl_dyn_access=>create( ir_data = lr_act iv_component = 'output-properties-conversion_flags_map-properties-is_value_list_converted').
    lr_val = lo_data->ref( ).

    cl_aunit_assert=>assert_bound( act = lr_val msg = 'Generation of deep structure with different pretty name modes fails!' ).

    lo_data = lcl_dyn_access=>create( ir_data = lr_act iv_component = 'output-required[1]').
    ...
    lo_data = lcl_dyn_access=>create( iv_data = lt_comp_descr iv_component = '[2]-name').
    lo_data->value( IMPORTING ev_data = lv_string ).

    cl_aunit_assert=>assert_equals( act = lv_string exp = 'FIELD2' msg = 'Dynamic access fails!' ).

    lo_data = lcl_dyn_access=>create( iv_data = lt_comp_descr iv_component = '[name=FIELD1]-suffix').
    lo_data->value( IMPORTING ev_data = lv_string ).
  ...
  METHOD deserialize_odata.
    ...
    CONCATENATE
    `{"d":{"__metadata":{"id":"https://abc.com:1304/sap/opu/odatazINTEROP/PersContainers(id='sap.ushell.UserDefaultParameter',category='P')",`
    `"uri":"https://abc.com:1304/sap/opu/odatazINTEROP/PersContainers(id='sap.ushell.UserDefaultParameter',category='P')","type":"INTEROP.PersContainer"},`
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = ls_odata_response ).
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = ls_odata_response2 ).
  ...
  METHOD initialize_on_deserialize.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case CHANGING data = ls_data ).
  ...
  METHOD deserialize_ref_to_data.
    ...
    zcl_json=>deserialize( EXPORTING json = lv_json pretty_name = zcl_json=>pretty_mode-camel_case assoc_arrays = abap_true assoc_arrays_opt = abap_true CHANGING data = ls_data ).
    cl_aunit_assert=>assert_bound( act = ls_data-struct msg = 'Deserialize to unknown REF TO data for struct fails!' ).

    lo_data = lcl_dyn_access=>create( iv_data = ls_data-struct iv_component = 'field1').
