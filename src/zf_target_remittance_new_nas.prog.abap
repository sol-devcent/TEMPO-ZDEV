REPORT zf_target_remittance_new_nas MESSAGE-ID zf NO STANDARD PAGE HEADING
                                    LINE-SIZE 262.
...
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS : rad2 RADIOBUTTON GROUP grp2.
...
SELECTION-SCREEN END OF LINE.
*PARAMETERS : radio2 RADIOBUTTON GROUP grp1 MODIF ID hid.
*SELECTION-SCREEN : COMMENT 5(45) text-004 FOR FIELD radio2.
*SELECTION-SCREEN END OF LINE.
*SELECTION-SCREEN BEGIN OF LINE.
*PARAMETERS : radio3 RADIOBUTTON GROUP grp1 MODIF ID hid.
...
SELECTION-SCREEN END OF LINE.
*SELECTION-SCREEN BEGIN OF LINE.
*PARAMETERS : radio5 RADIOBUTTON GROUP grp1 MODIF ID hid.
...
SELECTION-SCREEN : COMMENT 5(45) text-011 FOR FIELD radio8.
SELECTION-SCREEN END OF LINE.
...
*Perform f_cek_itab.
  ...
  ELSE.
    IF va_project = 'X'.
      ...
      ENDIF.
    ELSE.
      CONCATENATE text-008 pa_day 'day' INTO va_top SEPARATED BY space.
    ENDIF.
...
*    PERFORM f_write_header_column USING 'Branch and Industry'.
  ...
  IF radio17 = 'X' AND sy-ucomm = space.
    v_title2 = 'A/R Target Remittance Per Branch and Channel (National)'.
    PERFORM f_write_header11.
    PERFORM f_write_header_column1.
  ENDIF.

  IF radio4 = 'X' AND sy-ucomm = space.
    v_title2 = 'A/R Target Remittance Per Sub Customer Group (National)'.
*    PERFORM f_write_header1.
*    PERFORM f_write_header_column USING 'Customer Group'.
    PERFORM f_write_header12.
...
*    ENDIF.
    ...
    IF i_result61[] IS INITIAL.
      LOOP AT lt_t016t .
        ...
        i_result61-brsch = lt_t016t-brsch.
        LOOP AT i_result6 INTO wa_result WHERE bukrs = i_result61-bukrs AND
                                               brsch = i_result61-brsch.
          ...
          CLEAR: i_result61-target.
        ENDLOOP.
        LOOP AT i_result6_real INTO wa_result WHERE bukrs = i_result61-bukrs AND
                                                    brsch = i_result61-brsch.
...
* Select BSID for UMSKZ EQ SPACE
      SELECT a~bukrs a~gsber a~budat a~bldat a~gjahr a~belnr a~kunnr
                   ...
                   a~xref1 a~xref2 a~blart a~zterm
                   c~name1
...
*    ENDIF.
    ...
    IF i_result81[] IS INITIAL.
      LOOP AT lt_tvv3t .
        CLEAR: i_result81.
        i_result81-bukrs = pa_bukrs.
        i_result81-kvgr3 = lt_tvv3t-kvgr3.
        LOOP AT i_result8 INTO wa_result WHERE bukrs = i_result81-bukrs AND
                                               kvgr3 = i_result81-kvgr3.
          CLEAR: i_result81-target.
          i_result81-vkbur = wa_result-vkbur.
...
* cetak
  SORT i_result81 BY bukrs kvgr3 vkbur.
  LOOP AT i_result81.
    AT NEW kvgr3.
      CLEAR lt_tvv3t.
      READ TABLE lt_tvv3t WITH KEY kvgr3 = i_result81-kvgr3.
      CONCATENATE i_result81-kvgr3 lt_tvv3t-bezei
        INTO va_text SEPARATED BY '-'.
      va_texttotal2 = va_text.
    ENDAT.
    ...
    AT END OF kdgrp.
      SUM.
      i_result81-actual = ABS( i_result81-actual ).
      IF i_result81-target EQ 0.
        i_result81-persen = 0.
      ELSE.
        i_result81-persen = i_result81-actual / i_result81-target * 100.
