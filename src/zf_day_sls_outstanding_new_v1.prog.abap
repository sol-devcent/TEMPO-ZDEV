...
DATA: i_itab         TYPE t_itab OCCURS 0,
      ...
      i_result71     TYPE t_result OCCURS 0 WITH HEADER LINE,
      i_result81     TYPE t_result OCCURS 0,
      ...
      i_result72     TYPE t_result OCCURS 0,
      i_result82     TYPE t_result OCCURS 0,
      i_zfchanel     LIKE zfchanel OCCURS 0 WITH HEADER LINE,
      tmp_result72   TYPE t_result OCCURS 0,
      tmp_result71   TYPE t_result OCCURS 0,
      ...
      tmp_result82   TYPE t_result OCCURS 0,
      tmp_result81   TYPE t_result OCCURS 0,
      va_channel(1),
      va_flag(1),
      va_flag1(1),
      va_flag2(1),
...
AT USER-COMMAND.
  ...
  CASE sy-ucomm.
    WHEN 'BRANCH'.
      CLEAR: radio1,radio2,radio3,radio4,radio5,radio6,radio7,radio8,radio9.
      PERFORM f_proses1.
    WHEN 'CUSTOMER'.
      CLEAR: radio1,radio2,radio3,radio4,radio5,radio6,radio7,radio8,radio9.
      radio4 = 'X'.
...
FORM f_proses2.
  ...
  IF i_result21 IS INITIAL.
    ...
    IF wa_result-kdgrp NE space.
      ...
      CLEAR wa_result.
    ENDIF.
    SORT i_itab3 BY bukrs vkbur kdgrp.
...
FORM f_proses5.
  ...
  IF i_result51 IS INITIAL.
    ...
    IF wa_result-brsch NE space.
      ...
      CLEAR wa_result.
    ENDIF.
    SORT i_itab3 BY bukrs vkbur brsch.
...
*                     a~umsks eq space and
    ...
    ELSE.
      SELECT a~bukrs a~gsber a~budat a~bldat a~gjahr a~belnr a~kunnr
             a~blart a~zbd1t a~zfbdt a~zuonr a~dmbtr a~shkzg
             a~xref1 a~xref2 a~blart a~anln1
             b~name1 b~brsch c~kdgrp c~kvgr3
             p~vkbur
             INTO CORRESPONDING FIELDS OF TABLE i_itab_bsid
             FROM bsid AS a JOIN vbrp AS p ON p~vbeln EQ a~vbeln AND
                                              p~posnr EQ '000010'
      ...
      SELECT a~bukrs a~gsber a~budat a~bldat a~gjahr a~belnr a~kunnr
             ...
             p~vkbur
             INTO CORRESPONDING FIELDS OF TABLE i_itab_bsad
             FROM bsad AS a JOIN vbrp AS p ON p~vbeln EQ a~vbeln AND
                                              p~posnr EQ '000010'
                            JOIN kna1 AS b ON a~kunnr EQ b~kunnr
                            JOIN knvv AS c ON c~kunnr EQ a~kunnr AND
                                              c~vkorg EQ a~bukrs AND
...
*  * End Koreksi by budi 08/09/2005 req. by SJT
    ...
    ELSE.
      SELECT a~bukrs a~gsber a~budat a~bldat a~gjahr a~belnr a~kunnr
             a~blart a~zbd1t a~zfbdt a~zuonr a~dmbtr a~shkzg
             a~xref1 a~xref2 a~blart a~anln1
             b~name1 b~brsch c~kdgrp c~kvgr3 d~pernr
             p~vkbur
             INTO CORRESPONDING FIELDS OF TABLE i_itab1
             FROM bsid AS a JOIN vbrp AS p ON p~vbeln EQ a~vbeln AND
                                              p~posnr EQ '000010'
...
*  if wa_result1-outstanding ne 0.
  ...
  c1 = 0.
  WRITE: / sy-uline.
  CLEAR : wa_total-avrsales, wa_total-outstanding.
...
FORM f_proses7.
  ...
  IF va_channel IS INITIAL.
    IF i_result71 IS INITIAL.
      ...
      SORT i_itab3 BY bukrs vkbur channel kdgrp.
...
FORM f_new_proses2 .
  ...
  LOOP AT lt_itab1 INTO ls_itab1.
    ...
    LOOP AT lt_itab2 INTO ls_itab2 WHERE bukrs = ls_itab1-bukrs
      ...
      PERFORM f_calculate_data2 USING ls_itab2-bukrs ls_itab2-vkbur
                                      ls_itab2-kdgrp
...
*&---------------------------------------------------------------------*
FORM f_calculate_data2  USING    fu_bukrs fu_vkbur fu_kdgrp
...
FORM f_new_proses5 .
  ...
  LOOP AT lt_itab1 INTO ls_itab1.
    ...
    LOOP AT lt_itab2 INTO ls_itab2 WHERE bukrs = ls_itab1-bukrs
      ...
      PERFORM f_calculate_data5 USING ls_itab2-bukrs ls_itab2-vkbur
                                      ls_itab2-brsch
                                CHANGING lv_avrsales lv_outstanding
                                         lv_dso.
...
*&---------------------------------------------------------------------*
FORM f_calculate_data5  USING    fu_bukrs fu_vkbur fu_brsch
                        CHANGING fc_avrsales fc_outstanding fc_dso.
  DATA : ls_itab    LIKE LINE OF i_itab.
...
FORM f_new_proses7 .
  ...
  lt_itab2[] = lt_itab1[].
  SORT lt_itab2 BY bukrs vkbur.
  DELETE ADJACENT DUPLICATES FROM lt_itab2 COMPARING bukrs vkbur.
...
FORM f_proses7_kdgrp  TABLES   ft_itab STRUCTURE zf25n
  ...
  DATA : lt_zfchanel TYPE STANDARD TABLE OF zfchanel,
         ls_zfchanel LIKE LINE OF lt_zfchanel.
  ...
  lt_itab2[] = ft_itab[].
  LOOP AT i_zfchanel INTO ls_zfchanel WHERE vkbur = fu_vkbur.
    ls_itab2-bukrs    = ls_zfchanel-bukrs.
    ls_itab2-vkbur    = ls_zfchanel-vkbur.
    ls_itab2-channel  = ls_zfchanel-channel.
    ls_itab2-kdgrp    = ls_zfchanel-kdgrp.
    ls_itab2-brsch    = ls_zfchanel-brsch.
    APPEND ls_itab2 TO lt_itab2.
    CLEAR ls_itab2.
  ENDLOOP.
  SORT lt_itab2 BY bukrs vkbur channel kdgrp.
  DELETE ADJACENT DUPLICATES FROM lt_itab2 COMPARING bukrs vkbur channel kdgrp.
  lt_itab1[] = lt_itab2[].
  SORT lt_itab1 BY bukrs vkbur channel.
  DELETE ADJACENT DUPLICATES FROM lt_itab1 COMPARING bukrs vkbur channel.
  LOOP AT lt_itab1 INTO ls_itab1 WHERE vkbur = fu_vkbur.
    CLEAR : va_nou.
    LOOP AT lt_itab2 INTO ls_itab2 WHERE bukrs   = ls_itab1-bukrs
                                     AND vkbur   = ls_itab1-vkbur
      ...
      IF sy-subrc NE 0.
        t151t-ktext = 'Others'.
      ENDIF.
      ltext1-l_data1 = ls_itab2-channel.
      ltext1-l_data2 = space.
      ltext1-l_data3 = ls_itab2-kdgrp.
      ltext1-l_data4 = '.'.
      ltext1-l_data5 = t151t-ktext.
      ...
      PERFORM f_calculate_data7 USING 'KDGRP' ls_itab2-bukrs ls_itab2-vkbur
                                      ls_itab2-channel ls_itab2-kdgrp ''
                                CHANGING lv_avrsales lv_outstanding
                                         lv_dso.
    ...
    TRY .
        lv_dso = wa_subtotal1-outstanding / wa_subtotal1-avrsales.
      CATCH cx_sy_zerodivide.
    ENDTRY.
    ...
    WRITE AT c1(1) sy-vline NO-GAP. c1 = c1 + 1.
    PERFORM f_write_kosong1.
    CLEAR : wa_subtotal1-avrsales, wa_subtotal1-outstanding, lv_dso.
  ENDLOOP.
ENDFORM.
...
FORM f_proses7_brsch  TABLES   ft_itab STRUCTURE zf25n
  ...
  DATA : lt_itab1    TYPE STANDARD TABLE OF t_itab,
         ...
         ls_itab2    LIKE LINE OF lt_itab2,
         ls_itab     LIKE LINE OF ft_itab,
         ls_zfchanel LIKE LINE OF i_zfchanel.
  ...
  LOOP AT i_zfchanel INTO ls_zfchanel WHERE vkbur = fu_vkbur.
    ...
    CLEAR ls_itab2.
  ENDLOOP.
  SORT lt_itab2 BY bukrs vkbur channel brsch.
  DELETE ADJACENT DUPLICATES FROM lt_itab2 COMPARING bukrs vkbur channel brsch.
  ...
  LOOP AT lt_itab1 INTO ls_itab1 WHERE vkbur = fu_vkbur.
    ...
    LOOP AT lt_itab2 INTO ls_itab2 WHERE bukrs   = ls_itab1-bukrs
      ...
      PERFORM f_calculate_data7 USING 'BRSCH' ls_itab2-bukrs ls_itab2-vkbur
                                      ls_itab2-channel '' ls_itab2-brsch
                                CHANGING lv_avrsales lv_outstanding
                                         lv_dso.
...
*&---------------------------------------------------------------------*
FORM f_calculate_data7  USING    fu_field fu_bukrs fu_vkbur fu_channel
                                 fu_kdgrp fu_brsch
                        CHANGING fc_avrsales fc_outstanding fc_dso.
  DATA : ls_itab    LIKE LINE OF i_itab.
  ...
  LOOP AT lt_itab1 INTO ls_itab1.
    ...
    LOOP AT lt_itab2 INTO ls_itab2 WHERE bukrs = ls_itab1-bukrs
      ...
      SELECT SINGLE bezei
        ...
        INTO lv_bezei
        WHERE spras EQ sy-langu
          AND kvgr3 EQ ls_itab2-kvgr3.
      IF sy-subrc NE 0.
        lv_bezei = 'Others'.
      ENDIF.
      CLEAR ltext.
      CONCATENATE ls_itab2-kvgr3 lv_bezei
         INTO ltext SEPARATED BY '-'.
      WRITE AT c1(w1) va_nou NO-GAP. c1 = c1 + w1.
      WRITE AT c1(1) sy-vline NO-GAP. c1 = c1 + 1.
      WRITE AT c1(w2) ltext NO-GAP. c1 = c1 + w2.
      ...
      PERFORM f_calculate_data8 USING ls_itab2-bukrs ls_itab2-vkbur
                                      ls_itab2-kvgr3
                                CHANGING lv_avrsales lv_outstanding
                                         lv_dso.
...
*&---------------------------------------------------------------------*
FORM f_calculate_data8  USING    fu_bukrs fu_vkbur fu_kvgr3
                        CHANGING fc_avrsales fc_outstanding fc_dso.
  DATA : ls_itab    LIKE LINE OF i_itab.
